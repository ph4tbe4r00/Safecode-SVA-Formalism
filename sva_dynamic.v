...
Definition RegionStore := partial_map value. (* var -> value *)
...
Record State_exp : Type := state_e {
  ...
  L_exp : partial_map RegionT; (* nodevar -> RegionT *)
  ES_exp : exp (* expression *)
}.

Record State_stmt : Type := state_s {
  VEnv_stmt : partial_map value; (* var -> value *)
  L_stmt : partial_map RegionT; (* nodevar -> RegionT *)
  ES_stmt : stmt (* statement *)
}.

(* hack identity function right now ... *)
Definition updateR6 (l : partial_map RegionT) (v1 : value) (v2 : value) :=
  l.

(* hack just returning uninit ... *)
Definition getvalue (l : partial_map RegionT) (v : value) :=
  Val Uninit.
...
Inductive stmt_step : State_stmt -> State_stmt -> Prop :=
  ...
  | R3 : forall venv l x v,
      value_v v ->
      state_s venv l (Assign x (Val v)) ==>s state_s (extend venv x v) l Epsilon
  ...
  | R6 : forall venv l v1 v2 n rho,
      ...
      value_v v2 ->
      state_s venv l (Store (Val v2) (Val v1)) ==>s state_s venv (updateR6 l v1 v2) Epsilon
  | R6char : forall venv l v1 v2 n rho,
      value_v v1 ->
      v1 = (Int n) \/ v1 = (Region rho) ->
      value_v v2 ->
      state_s venv l (Storec (Val v2) (Val v1)) ==>s state_s venv (updateR6 l v1 v2) Epsilon
  ...
  | R9 : forall venv l v1 v2 e3 venv' l' e3',
      ...
      state_s venv l (StoreToU (Val v1) (Val v2) e3) ==>s state_s venv' l' (StoreToU (Val v1) (Val v2) e3')
  | R9char : forall venv l v1 v2 e3 venv' l' e3',
      value_v v1 ->
      value_v v2 ->
      state_e venv l e3 ==>e state_e venv' l' e3' ->
      state_s venv l (StorecToU (Val v1) (Val v2) e3) ==>s state_s venv' l' (StorecToU (Val v1) (Val v2) e3')
  (* R10 need update ... *)
  | R11 : forall venv l v1 v2 rho,
      value_v v1 ->
      value_v v2 ->
      state_s venv l (StorecToU (Val (Region rho)) (Val v1) (Val v2)) ==>s state_s venv (updateR6 l v1 v2) Epsilon
  ...
  | R14 : forall venv l v n rho f rs,
      ...
      l rho = Some (region_t f rs) ->
      state_s venv l (PoolFree (Val (Region rho)) (Val v)) ==>s 
      state_s venv (extend l rho (region_t (v::f) rs))Epsilon
  | R15 : forall venv l rho tau x s,
      l rho = None ->
      state_s venv l (PoolInit rho tau x s) ==>s state_s (extend venv x (Region rho)) (extend l rho (region_t [] (empty))) (PoolPop s rho)
  ...
  | R17 : forall venv l x rho r,
      state_s (extend venv x (Region rho)) (extend l rho r) (PoolPop Epsilon rho) ==>s
...
with stmt_exp : State_exp -> State_exp -> Prop :=
  ...
  *)
  ...
  | R34 : forall venv l rho f rs a,
      state_e venv (extend l rho (region_t (a::f) rs)) (PoolAlloc (Val (Region rho)) (Val (Int 1))) ==>e
      state_e venv (extend l rho (region_t f rs)) (Val a)
  (* R34, R35 and R36 are ... *)
